...
from brains.sources.vault.indexing import pointer_manifest_path, write_active_index_pointer
...
def make_paths(tmp_path: Path) -> BrainsPaths:
    ...
    index_root = brains_root / ".index" / "vault_search"
    return BrainsPaths(
        repo_root=repo_root,
...
def make_index_config(paths: BrainsPaths) -> VaultIndexConfig:
...
def test_write_active_index_pointer_for_fallback_index(tmp_path: Path) -> None:
    ...
    assert pointer_path == tmp_path / ".brains" / ".index" / "vault_search" / "active_index.json"
