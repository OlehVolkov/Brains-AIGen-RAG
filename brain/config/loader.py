...
from brain.config.models import BrainConfig, BrainPaths, ResearchPaths
from brain.config.sources import CombinedTomlSource
...
def config_path() -> Path:
    return brain_root() / "config" / "brain.toml"


def local_config_path() -> Path:
    return brain_root() / "config" / "local.toml"


def dotenv_path() -> Path:
    return brain_root() / ".env"
...
class DefaultBrainConfig(BrainConfig):
    @classmethod
...
def load_config(
    *,
    ...
    class CustomBrainConfig(BrainConfig):
        @classmethod
        def settings_customise_sources(
            cls,
            settings_cls: type[BaseSettings],
