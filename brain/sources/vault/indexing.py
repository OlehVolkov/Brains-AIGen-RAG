...
from brain.shared.text import chunk_id
from brain.config.loader import get_config, resolve_repo_path
from brain.sources.vault.markdown import list_markdown_paths, load_markdown_documents
...
def write_manifest(
    ...
    manifest = {
        ...
        "db_uri": config.paths.db_uri.relative_to(config.paths.brain_root).as_posix()
        if config.paths.db_uri.is_relative_to(config.paths.brain_root)
